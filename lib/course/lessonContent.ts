    ...
    sections: [
      {
        ...
        bullets: [
          "音程(interval):兩個音之間的距離,用「度」來稱呼(如三度、五度)。",
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "以 A 為主音:A 小調五聲是 A C D E G,加入降五度 Eb 之後,A 小調藍調 = A C D Eb E G(六個音)。注意這裡 Eb(b5)夾在 D 與 E 之間,實際彈奏時常把它當成經過音,從 D 或 E 短暫滑過,而不是長時間停留。",
        ],
        bullets: [
          "藍調音階 = 小調五聲 + 降五度(b5,blue note)。",
          "A 小調藍調 = A C D Eb E G(六個音)。",
          "藍調音(Eb)常作為經過音,短暫滑過、不久留。",
        ],
        tool: {
    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "在 C 大調裡,就是 Cmaj7、Dm7、Em7、Fmaj7、G7、Am7、Bm7b5。其中 V7(G7)帶有最強的「想回到主和弦」的張力。原因在於它內部的三全音:G7 的三度 B 與小七度 F 相距一個三全音(tritone),這個極不穩定的音程會想要解決——B 往上半音收到 C、F 往下半音收到 E,正好落進主和弦 C 的根音與三度,於是產生強烈的「回家」感。這是和聲進行裡的關鍵推進力,也是下一個重點。",
        ],
        bullets: [
    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "經濟手法(economy)則是「能少動就少動」:換弦時手指走最短路徑、按過的手指不急著抬太高、撥弦方向順著移動方向。它是建立在交替撥弦穩固之後的「進階精修」,不是初學就要追的目標——先把交替撥弦練扎實,再用經濟手法把多餘動作省掉,速度與穩定度都會跟著上來。",
        ],
        bullets: [
      ...
      },
      {
        heading: "一弦三音與模進",
        paragraphs: [
          "一弦三音(three-notes-per-string,3NPS)指型,是把音階安排成每一條弦上剛好彈三個音的指法。比起一條弦兩個音的傳統盒型,它讓手指排列更規律、跨弦更順,因此特別利於圓滑奏(legato)與提升速度;它和音階課裡的「把位」觀念是相呼應的——同一個音階,只是換一種更適合跑動的指法來框它。",
          "模進(sequence)則是一種練法:把一個小小的音型(例如「四個音一組」),沿著音階一級一級往上(或往下)反覆移動。例如在音階上彈 1-2-3-4,再從第二級彈 2-3-4-5,接著 3-4-5-6……。模進很適合搭配 3NPS 與節拍器:先用慢速把音型彈乾淨,再循序加速,既練手指規律也練耳朵對音階的熟悉度。",
        ],
        bullets: [
          "一弦三音(3NPS):每條弦放三個音,跨弦順、利於圓滑奏與速度。",
          "與音階課的把位相呼應:同一音階的另一種指法。",
          "模進(sequence):把小音型沿音階級進反覆移動(如 1-2-3-4 → 2-3-4-5)。",
          "用 3NPS + 節拍器練模進,先慢求準再循序加速。",
        ],
        tool: {
          label: "用節拍器替模進設定速度與細分",
          href: "/practice",
        },
      },
      {
        heading: "節拍器:速度與細分",
        paragraphs: [
          "節拍器給你一個不會偏移的拍點,是練基本功的核心工具。先講清楚「拍」:節拍器每響一聲就是一拍,而速度(BPM,每分鐘拍數)就是一分鐘有幾響。先把速度設在「能彈得乾淨」的慢速,確認穩定後,每次只加一點點(例如 5–10 BPM),讓進步可被量化。",
