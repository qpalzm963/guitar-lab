...
| 路徑 Route | 名稱 | 說明 |
| --- | --- | --- |
| `/course` | 課程 | 循序漸進的 **7 堂課**(基本功 → 音程 → 音階 → 和弦 → 順階和弦 → CAGED → 即興),含章節大綱、對應工具連結、學習進度,以及原創的選擇題小測驗。 |
| `/curriculum` | 課程地圖 | **六大領域**(音階／和弦／節奏訓練／技巧／樂句・編曲・器材／樂理)共 **111 個項目**,記錄已學進度並連到工具;尚無工具的項目標示為「規劃中」。 |
...
| `/intervals` | 音程練習 | 看音程在指板上的分布,並做視覺辨認測驗(visual quiz);可用 Tone.js 播放音程(上行／下行／同時)。 |
...
| `/practice` | 練習工具 | 節拍器(metronome)、持續音(drone)、**調音器**(麥克風即時音高偵測)與**聽力測驗**(辨認音程／和弦),練節奏、音準與辨音。 |
...
- **音訊**:[`tone`](https://tonejs.github.io)(節拍器、drone 與聽力測驗,client-only、動態 `import` 延遲載入)+ 原生 Web Audio(調音器:麥克風 → AnalyserNode → NSDF 自相關音高偵測)
...
- **測試**:[`vitest`](https://vitest.dev) —— 目前 **285** 個測試,全部通過(21 個測試檔)
...
├─ lib/
│  ├─ theory/           # 樂理運算:notes / intervals / chords / caged / positions / fretboard / harmony / scaleProjection / pitchMath(以 tonal 為底)
...
│  ├─ audio/            # 節拍器 / drone / 音符播放器 / 聽力測驗(Tone.js)+ 調音器音高偵測(pitch / tuner,原生 Web Audio);純邏輯與引擎分離以便測試
